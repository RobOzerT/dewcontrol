...
void hvacontrol::selftest() {
  float valveV = checkValveVoltage(_valvecontrolPin);
  if (valveV ==0){
    Serial.println("Voltage ERROR: Valve");
    fault(1);
  }

  float waterTemp_mA = checkSensorCurrent(WaterTempPin);
  if (waterTemp_mA == 0) {
    Serial.println("Ma ERROR: waterTemp");
    fault(2); 
  }
  
  // float RH_mA = checkSensorCurrent(RHPin);
  // if (RH_mA == 0) {
  //   Serial.println("Ma ERROR:RH");
  //   fault(3); 
  // }
  
  // float airTemp_mA = checkSensorCurrent(AirTempPin);
  // if (airTemp_mA == 0) {
  //   Serial.println("Ma ERROR: airTemp");
  //   fault(4); 
  // }
}

float hvacontrol::checkValveVoltage(int valvePin){
  int numrepetition = 3; // Number of repetitions to check
  float Voltage = 0.0;
  float minV = 0.0;
  float valveOut = analogRead(valvePin);
  int FaultReadinCount = 0;

  for (int i = 0; i < numrepetition; i++) {
    Voltage = analogRead(valvePin);
    Voltage = valveOut * 2.0;
    if(Voltage<minV){
      FaultReadinCount++;
    }
    else{
  ...
  if (FaultReadinCount == numrepetition){
    Voltage = 0;
  }
  return Voltage;
...
float hvacontrol::checkSensorCurrent(int sensorpin) {
  int numrepetition = 3; // Number of repetitions to check
  float minCurrent = 4.0; // Minimum current [mA]
  float maxV = 5.0; // Maximum voltage [V]
  float resistorValue = 250.0; // Resistor value [ohm]

  int FaultReadingCount = 0; 
  float current = 0.0;
  for (int i = 0; i < numrepetition; i++){
  ...
  if (FaultReadingCount == numrepetition) {
    current = 0;
  }
